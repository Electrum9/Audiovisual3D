...
def load_bird(path_to_dataset):
    rir_files = list(p for p in Path(path_to_dataset).rglob("*.flac") if not p.name.startswith('.'))
...
def load_speech_dataset():
    ...
    speech_files = list(p for p in Path("./dataset/DAPS").rglob("*.wav") if not p.name.startswith('.'))
